#!/usr/bin/env python3
"""
metis-write-proposal.py — the ONLY sanctioned write path for Metis in PROPOSE mode.

Sibling of hermes-write-proposal.py. Validates a proposal payload against Metis's hard
constraints:
- Single proposed_variable (not a list)
- Single proposed_value (no multiple blocks)
- scope ∈ config.allowed_scopes (read from Meta/metis/config.json)
- --variable must be in metis_owns for --scope per surface-manifest.yaml
- --surface-category must be 'learned_param' (Metis exclusively owns learned-param surface)
- ≥config.min_evidence_citations evidence paths
- ≤config.max_proposals_per_day already written today

If valid, writes a proposal artifact to Meta/metis-proposals/queued/<date>.md
and posts a ⏳ jarvis line to Meta/agent-messages.md requesting review.

The artifact always includes times_observed and confidence from the notebook observation
so Step 5h Metric A measurement can check (times_observed < 3 OR confidence < high) at
fire time.

Flag aliases (old flags emit DEPRECATED warning; both accepted):
...
New required flags (no aliases):
  ...
  --times-observed             int ≥ 1 (from notebook observation at fire time)

Usage (called by Metis via Bash):
    python3 metis-write-proposal.py \\
        --scope my-project \\
        --variable calibrator.A \\
        ...
        --surface-category learned_param \\
        --times-observed 2 \\
...
CONFIG = VAULT / "Meta" / "metis" / "config.json"
QUEUED_DIR = VAULT / "Meta" / "metis-proposals" / "queued"
...
def load_config() -> dict:
    if not CONFIG.exists():
        sys.stderr.write(
            f"[metis-proposal] REJECT: Metis config missing at {CONFIG}. "
            "Run Step 4 Track B setup to create Meta/metis/config.json.\n"
        )
        sys.exit(1)
...
def load_manifest() -> dict:
    ...
    if not SURFACE_MANIFEST.exists():
        sys.stderr.write(
            f"[metis-proposal] REJECT: surface-manifest.yaml missing at {SURFACE_MANIFEST}. "
    ...
    try:
        ...
        sys.stderr.write(
            f"[metis-proposal] REJECT: surface-manifest.yaml malformed: {e}\n"
        )
        sys.exit(1)
    if not isinstance(data, dict):
        sys.stderr.write(
            "[metis-proposal] REJECT: surface-manifest.yaml must be a mapping at top level.\n"
        )
        sys.exit(1)
    return data
...
class _DeprecationAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        if option_string and option_string != self._canonical_flag:
            sys.stderr.write(
                f"[metis-proposal] DEPRECATED: {option_string!r} is deprecated. "
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Metis proposal writer — see module docstring for usage."
    )
    ...
    ap.add_argument(
        ...
        type=int,
        required=True,
        help="Times observed count from the notebook observation at fire time (required for Step 5h Metric A).",
    )
    ...
    ap.add_argument("--confidence", choices=["low", "med", "medium", "high"], required=True)
    ap.add_argument(
...
def validate_manifest_ownership(manifest: dict, scope: str, variable: str, surface_category: str) -> int:
    """
    Validate that:
      1. The variable is in metis_owns for the given scope (not hermes_owns or forbidden).
      2. The declared surface_category is 'learned_param' (Metis only writes learned_param proposals).
    Returns 0 on pass, 1 on failure (writes to stderr).
    """
    scope_data = manifest.get(scope)
    if scope_data is None:
        sys.stderr.write(
            f"[metis-proposal] REJECT: scope {scope!r} not found in surface-manifest.yaml. "
    ...
    if variable in forbidden:
        sys.stderr.write(
            f"[metis-proposal] REJECT: variable {variable!r} is in the forbidden list for "
            f"scope {scope!r}. It may not be proposed by any agent.\n"
        )
        return 1

    if variable in hermes_owns:
        sys.stderr.write(
            f"[metis-proposal] REJECT: variable {variable!r} is owned by Hermes (hermes_owns) "
            f"for scope {scope!r}. Use hermes-write-proposal.py instead.\n"
        )
        return 1

    if variable not in metis_owns:
        sys.stderr.write(
            f"[metis-proposal] REJECT: variable {variable!r} is not listed in metis_owns for "
            f"scope {scope!r}. Add it to the manifest via AR Director, or use hermes-write-proposal.py "
            "if it is a strategy-level parameter.\n"
        )
        return 1

    # surface_category must be "learned_param" for metis_owns variables
    if surface_category != "learned_param":
        sys.stderr.write(
            f"[metis-proposal] REJECT: variable {variable!r} is in metis_owns which maps to "
            f"surface_category='learned_param', but caller passed --surface-category={surface_category!r}. "
            "Fix the surface_category or move the variable to hermes_owns.\n"
        )
        return 1
...
def _emit_inbox_item(
    ...
    """
    Emit an ADR-006 observer-apply/v1 inbox item for a metis proposal that is
    allow-listed AND has cleared the promotion threshold.
    Returns the written path, or None if allow-list absent.
    """
    ...
    inbox_item: dict = {
        ...
        "diff": {
            ...
            else str(VAULT / "Meta" / "metis" / "config.json"),
            "diff_kind": "config_knob",
            "applier": "observer-apply.py",
            "patch": patch_op,
            "patch_sha256": patch_sha256,
        },
        "blast_radius": f"Single observer-config learned_param for scope={args.scope}. "
            "Touches no gate, no review depth, no trading code.",
        "rollback": f"Revert to current_value={args.current_value!r}.",
        "rate_limit_token": f"{now.date().isoformat()}-metis-1",
        "contrarian_receipt": "",
    }
...
def main() -> int:
    ...
    # Post-parse required checks for the aliased flags
    if args.current_value is None:
        sys.stderr.write("[metis-proposal] error: --current-value (or --baseline-value) is required\n")
        return 1
    if args.success_criterion is None:
        sys.stderr.write("[metis-proposal] error: --success-criterion (or --hypothesis) is required\n")
        return 1
    if args.rollback_condition is None:
        sys.stderr.write("[metis-proposal] error: --rollback-condition (or --falsification) is required\n")
        return 1

    if args.times_observed < 1:
        sys.stderr.write("[metis-proposal] REJECT: --times-observed must be ≥ 1\n")
        return 1
    ...
    if "," in args.variable:
        sys.stderr.write("[metis-proposal] REJECT: proposed_variable must be a single name\n")
        return 1
    if ";" in args.proposed_value or "\n" in args.proposed_value:
        sys.stderr.write("[metis-proposal] REJECT: proposed_value must be a single literal\n")
        return 1

    # ── Validate scope lock ────────────────────────────────────────────────────
    if args.scope not in cfg.get("allowed_scopes", []):
        sys.stderr.write(
            f"[metis-proposal] REJECT: scope {args.scope!r} not in allowed_scopes "
    ...
    min_ev = cfg.get("min_evidence_citations", 3)
    if len(evidence) < min_ev:
        sys.stderr.write(
            f"[metis-proposal] REJECT: only {len(evidence)} evidence citations; "
            f"need ≥{min_ev}\n"
        )
        return 1

    # ── Daily proposal cap (null/None/0 = unlimited) ──────────────────────────
    ...
    if max_today and max_today > 0 and today_count >= max_today:
        sys.stderr.write(
            f"[metis-proposal] REJECT: daily cap {max_today} reached ({today_count} already today)\n"
        )
        return 3
    ...
    if not rat_path.exists():
        sys.stderr.write(f"[metis-proposal] REJECT: rationale file missing: {rat_path}\n")
        return 1
    rationale = rat_path.read_text(encoding="utf-8").strip()
    if len(rationale.split()) > 500:
        sys.stderr.write(
            "[metis-proposal] WARN: rationale > 500 words; truncating logically allowed "
    ...
    cycle_id = f"metis-{timestamp}-{today_count + 1:02d}"
    ...
    front = [
        ...
        f"times_observed: {args.times_observed}",
    ...
    notify_path.write_text(
        f"## [{now.strftime('%Y-%m-%d %H:%M')}] — From: metis → TO: Jarvis\n"
    ...
    with open(CHANGE_LOG, "a", encoding="utf-8") as f:
        ...
        try:
            f.write(
                f"[{now.strftime('%Y-%m-%d %H:%M')}] metis → PROPOSAL {cycle_id} "
    ...
    inbox_path = _emit_inbox_item(
        ...
        evidence=evidence,
        now=now,
        times_observed=args.times_observed,
        distinct_days=1,
        confidence=args.confidence,
    )
    if inbox_path is not None:
        print(f"[metis-proposal] INBOX item emitted at {inbox_path} (allow-listed for self-apply)")

    print(f"[metis-proposal] WROTE {out_path}")
    print(f"[metis-proposal] queued notify for Jarvis at {notify_path}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.write(f"[metis-proposal] FATAL: {e}\n")
        import traceback
        traceback.print_exc()
        sys.exit(2)
