#!/usr/bin/env python3
"""
hermes-append-notebook.py — the ONLY sanctioned write path for Hermes in OBSERVE mode.

Hermes is brain-only — its agent definition excludes Write/Edit/Task tools.
...
Usage (called by Hermes via Bash):
    python3 hermes-append-notebook.py append-observation \\
        --trigger <event-name> \\
        --hypothesis "<one-sentence>" \\
        --evidence "<path1>,<path2>" \\
        --confidence low|med|high

    python3 hermes-append-notebook.py reinforce-observation \\
        --pattern-id <slug-matching-existing-obs>

    python3 hermes-append-notebook.py update-status \\
        --pattern-id <slug> \\
        --new-status "proposal queued <id>"

    python3 hermes-append-notebook.py digest-overflow
...
NOTEBOOK_ARCHIVE = VAULT / "Meta" / "hermes" / "notebook.archive.md"
CHANGE_LOG = VAULT / "Meta" / "change-log.md"
SURFACE_MANIFEST = VAULT / "Meta" / "hermes" / "surface-manifest.yaml"

# Validation: only paths under Meta/hermes/ are acceptable as targets.
...
def check_variables_touched(variables_touched: list[str]) -> None:
    ...
    if not flat_index:
        sys.stderr.write(
            "[hermes-notebook] WARN: surface-manifest.yaml missing or empty — "
    ...
    for name in variables_touched:
        if name not in flat_index:
            sys.stderr.write(
                f"[hermes-notebook] WARN: variable {name!r} not registered in "
...
def append_observation(trigger: str, hypothesis: str, evidence: list[str], confidence: str, variables_touched: list[str] | None = None) -> int:
    if confidence not in ("low", "med", "high"):
        sys.stderr.write(f"[hermes-notebook] invalid confidence: {confidence}\n")
        return 1
    if len(hypothesis) < 10 or len(hypothesis) > 300:
        sys.stderr.write("[hermes-notebook] hypothesis must be 10-300 chars\n")
        return 1
    vt = variables_touched or []
    check_variables_touched(vt)
...
def _read_block_status(text: str, pattern_id: str) -> str:
    ...
    """
    # Capture everything in the block from Pattern id: through the next block boundary
    ...
    # Find ALL Status: lines in this block; last one wins (handles Reinforcement notes)
...
def reinforce_observation(
    pattern_id: str,
    variables_touched: list[str] | None = None,
    ...
    pat = re.compile(rf"(Pattern id: {re.escape(pattern_id)}(?![\w-]).*?Times observed:\s*)(\d+)(.*?Last observed:\s*)([^\n]+)", re.DOTALL)
    m = pat.search(text)
    if not m:
        sys.stderr.write(f"[hermes-notebook] pattern_id not found: {pattern_id}\n")
        return 1
    ...
    if evidence:
        ev_pat = re.compile(rf"(Pattern id: {re.escape(pattern_id)}(?![\w-]).*?Evidence cited:\s*)([^\n]+)", re.DOTALL)
...
def update_status(pattern_id: str, new_status: str) -> int:
    text = NOTEBOOK.read_text(encoding="utf-8")
    pat = re.compile(rf"(Pattern id: {re.escape(pattern_id)}(?![\w-]).*?Status:\s*)([^\n]+)", re.DOTALL)
...
def update_cursor(last_event_id: str) -> int:
    """Update notebook frontmatter `last_event_id` so the next Hermes invocation
    knows where to resume scanning new events from. Called by Hermes at the end
    of every OBSERVE or PROPOSE pass.
    """
    if not last_event_id or len(last_event_id) > 200:
        sys.stderr.write("[hermes-notebook] invalid last_event_id\n")
        return 1
    text = NOTEBOOK.read_text(encoding="utf-8")
    # Update last_event_id + last_invocation in frontmatter
    text = re.sub(
        r"^last_event_id:\s*\S+",
...
def digest_overflow() -> int:
    # Triggered by Hermes when notebook entry count > notebook_max_entries (200).
    # Move oldest 50 observations to archive; replace with a summary.
    text = NOTEBOOK.read_text(encoding="utf-8")
    ...
    with open(NOTEBOOK_ARCHIVE, "a", encoding="utf-8") as f:
        ...
        for b in to_archive:
            f.write(b.rstrip() + "\n\n")
    # Rebuild notebook: header + remaining observations + a digest summary
    header_match = re.search(r"^---[\s\S]*?---\s*<!--[\s\S]*?-->\s*## Observations\s*", text)
    ...
    digest = (
        ...
        f"50 oldest observations folded into archive; review notebook.archive.md if you need them.\n"
    )
...
def log_event(msg: str) -> None:
    try:
        with open(CHANGE_LOG, "a", encoding="utf-8") as f:
            ...
            try:
                f.write(f"[{now_str()}] hermes → NOTEBOOK {msg}\n")
            finally:
...
def main() -> int:
    ...
    p1.add_argument(
        "--variables-touched",
        default="",
        help="Comma-separated variable names this observation covers. Required by agent definition.",
    )
    ...
    p2.add_argument(
        "--variables-touched",
        default="",
        help="Comma-separated variable names this reinforcement covers.",
    )
    p2.add_argument(
        "--evidence",
        default="",
    ...
    if args.cmd == "append-observation":
        ...
        return append_observation(
            args.trigger, args.hypothesis, evidence, args.confidence,
...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.stderr.write(f"[hermes-notebook] FATAL: {e}\n")
        sys.exit(2)
