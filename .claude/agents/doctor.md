...
5. Relevant service logs in `Meta/sync/` (last 10 lines each) — check for any sync, compile, or cron logs your setup generates; read whichever are present
6. Additional runtime logs in `Meta/sync/` — if present
...
**Local crons** (parse the last timestamp from each log your setup generates; compare to `now`; apply baselines from `Meta/doctor/baselines.md`):
```bash
# Read whichever sync or compile logs exist in Meta/sync/
ls Meta/sync/*.log 2>/dev/null && tail -5 Meta/sync/*.log
```
...
| Cron | Primary local sync cron stale > 20 min | -10 |
| Cron | Secondary local sync cron stale > 20 min | -5 |
...
|------|----------|----------|-----|--------|
| primary-sync (local) | configure per your setup | HH:MM | Xm | OK/WARN/CRIT |
| secondary-sync (local) | configure per your setup | HH:MM | Xm | OK/WARN/CRIT |
