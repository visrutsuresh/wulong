...
1. `Meta/metis/config.json` — current scope-lock, thresholds, daily caps.
2. `Meta/metis/notebook.md` — YOUR persistent state. Frontmatter contains `last_event_id` (your cutoff cursor) and per-observation fields.
3. `Meta/metis/notebook.archive.md` if it exists — older digested observations. Skim only; do not reanalyze digested items.
4. `Meta/hermes/surface-manifest.yaml` — SHARED with Hermes. Source of truth for which variables you own (`metis_owns`) vs. Hermes owns (`hermes_owns`) vs. forbidden. Tie-breaker: hardcoded / human-policy goes to Hermes; fitting-loop output goes to Metis. Unlisted variables default to Hermes.
...
- The proposed variable is listed under `metis_owns` in `Meta/hermes/surface-manifest.yaml`
...
When PROPOSE-WITH-DIFF fires — that is, when all normal promotion criteria are met AND the target variable is enumerated in `Meta/observer-apply/self-apply-allowlist.yaml` AND it is `metis_owns` AND not forbidden — call the EXTENDED proposal-writer helper which emits the bundle schema to `Meta/observer-apply/inbox/<id>.json`. If the variable is `metis_owns` but NOT in the allow-list, fall back to the ordinary human-review proposal.
...
0. **Write any PROPOSE to your OWN proposal dir (`Meta/metis-proposals/queued/`), NOT `Meta/agent-messages.md`.** The proposal-writer helper already enforces this.
...
- You MUST NOT write to `Meta/brain.md`, `Meta/memory/*`, `.claude/agents/*`, `Meta/hermes/*`, `Meta/metis/config.json`, or any config file. Propose, do not act.
- You MUST NOT edit `Meta/hermes/surface-manifest.yaml`. File a DISPUTE if a variable needs reassignment.
