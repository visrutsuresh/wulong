#!/usr/bin/env python3
"""
cerebrum-search.py — BM25 full-text search over Meta/ knowledge documents.

The Cerebrum substrate is the vault's long-term memory layer: receipts, playbooks,
and the knowledge-base together form the ground truth of what the autonomous
agent system knows and how it operates. This script indexes all three tiers into an
...
Usage:
  python3 cerebrum-search.py "search query"
  python3 cerebrum-search.py --rebuild
  python3 cerebrum-search.py --update
  python3 cerebrum-search.py --stats
  python3 cerebrum-search.py --tier kb "search query"
  python3 cerebrum-search.py --root /path/to/vault "search query"
...
def _resolve_root(cli_root: Optional[str] = None) -> str:
    ...
    if cli_root:
        return cli_root
    # Meta/sync/ is two levels down from vault root
...
def _make_tiers(vault_meta: Path) -> dict[str, Path]:
    """Return the tier-name -> directory mapping given the Meta/ path."""
    return {
...
def _make_snippet(body: str, query_terms: list[str], lines: int = 2) -> str:
    """Find the passage with highest density of query_terms and return at most `lines` lines."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--tier",
        choices=["receipts", "playbooks", "kb"],
        default=None,
        help="Filter results to a single tier",
    )
    parser.add_argument("--rebuild", action="store_true", help="Rebuild index from scratch")
    parser.add_argument("--update", action="store_true", help="Incremental update (mtime-based)")
